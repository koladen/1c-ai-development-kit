...
name: mxl-compile
description: >
  Этот скилл MUST быть вызван когда нужно создать макет печатной формы (MXL-табличный документ) из JSON-определения.
  SHOULD также вызывать при создании новых печатных форм для обработок или отчётов.
  Do NOT использовать для декомпиляции MXL — используй mxl-decompile; для анализа MXL — используй mxl-info.
