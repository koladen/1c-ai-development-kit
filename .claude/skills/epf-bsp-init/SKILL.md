...
name: epf-bsp-init
description: >
  Этот скилл MUST быть вызван когда нужно добавить функцию регистрации БСП (СведенияОВнешнейОбработке) в модуль объекта обработки.
  SHOULD также вызывать когда внешняя обработка должна быть распознана подсистемой БСП.
  Do NOT использовать для добавления команд в регистрацию — используй epf-bsp-add-command.
